...
class KnowledgeGraph:
    ...
    def traverse(
        ...
        results = []
        visited = set()
        queue: list[tuple[str, list[tuple[KnowledgeNode, KnowledgeEdge]]]] = [(start_id, [])]
...
class GraphQueries:
    ...
    def similar_programs(self, program_name: str, limit: int = 5) -> list[dict]:
        ...
        return [{"id": n.id, "name": n.name, "type": n.type, "strength": e.strength} for n, e in neighbors[:limit]]
    ...
    def full_system_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "successful_outcomes": sum(1 for o in outcomes if o.properties.get("success")),
            "total_rewards": sum(o.properties.get("reward", 0) for o in outcomes),
        }
```
    ...
    async def fetch(self, opportunity: ScoredOpportunity, depth: str = "standard") -> ContextFragment | None:
        ...
        if similar:
            ...
            for node, edge in similar[:5]:
                text_lines.append(
                    f"- {node.name} ({node.properties.get('cycle', '')}, EV: ${node.properties.get('reward', 0):.2f})"
                )
            text = "\n".join(text_lines)
