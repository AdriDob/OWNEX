...
class StateMachine:
    ...
    def transition(
        ...
        if not self.can_transition(new_state):
            logger.warning(f"Cannot transition {self.opportunity_id}: {self.current_state.value} → {new_state.value}")
            return False
...
class StateEngine:
    ...
    def get_or_create(self, opportunity_id: str) -> StateMachine:
        ...
        if opportunity_id not in self._machines:
            self._machines[opportunity_id] = StateMachine(opportunity_id=opportunity_id)
    ...
    def get_state_summary(self) -> dict[str, int]:
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.execute("SELECT current_state, COUNT(*) FROM state_machines GROUP BY current_state")
