module.exports=[343379,e=>{"use strict";var t=e.i(899378);function n(e){return{agent_id:e.agent_id,dns_enabled:1===e.dns_enabled,cert_trusted:1===e.cert_trusted,setup_completed:1===e.setup_completed,last_started_at:e.last_started_at,last_error:e.last_error}}function r(e){let r=(0,t.getDbInstance)().prepare("SELECT * FROM agent_bridge_state WHERE agent_id = ?").get(e);return r?n(r):null}e.s(["getAgentBridgeState",0,r,"getAllAgentBridgeStates",0,function(){return(0,t.getDbInstance)().prepare("SELECT * FROM agent_bridge_state ORDER BY agent_id ASC").all().map(n)},"setLastError",0,function(e,n){(0,t.getDbInstance)().prepare(`INSERT INTO agent_bridge_state (agent_id, last_error)
     ...
     DO UPDATE SET score = score + excluded.score, updated_at = datetime('now')`).run(e,t,n)}])},795769,e=>{"use strict";var t=e.i(666680),n=e.i(899378),r=e.i(529646);function a(e){let t=(0,n.getDbInstance)().prepare("SELECT * FROM relay_tokens WHERE id = ?").get(e);return t?{...(0,r.rowToCamel)(t),enabled:1===t.enabled}:null}e.s(["checkRateLimit",0,function(e,t){let a=(0,n.getDbInstance)(),o=t;if(!o){let t=a.prepare("SELECT * FROM relay_tokens WHERE id = ?").get(e);if(!t)return{allowed:!1,remaining:0,resetIn:0};o=(0,r.rowToCamel)(t)}let s=Math.floor(Date.now()/1e3),i=60*Math.floor(s/60),l=86400*Math.floor(s/86400),c=a.prepare("SELECT request_count, cost FROM relay_rate_limits WHERE token_id = ? AND window_start = ?").get(e,i),u=c?.request_count||0;if(u>=o.maxRequestsPerMinute)return{allowed:!1,remaining:0,resetIn:60-s%60};let p=a.prepare("SELECT SUM(request_count) as total FROM relay_rate_limits WHERE token_id = ? AND window_start >= ?").get(e,l),d=p?.total||0;return d>=o.maxRequestsPerDay?{allowed:!1,remaining:0,resetIn:86400-s%86400}:{allowed:!0,remaining:Math.min(o.maxRequestsPerMinute-u,o.maxRequestsPerDay-d),resetIn:60-s%60}},"createRelayToken",0,function(a){let o=(0,n.getDbInstance)(),s="rl_"+(0,t.randomBytes)(16).toString("hex"),i="relay_"+(0,t.randomBytes)(24).toString("hex"),l=function(t){let{createHash:n}=e.r(666680);return n("sha256").update(t).digest("hex")}(i),c=Math.floor(Date.now()/1e3),u="rl_"+i.slice(6,14);o.prepare(`
  ...
  0
       ...
       WHERE ${n} > unixepoch('now') ORDER BY created_at DESC LIMIT 1`).get();return{totalEntries:r.total_entries,totalChars:r.total_chars,byProvider:o,byModel:i,oldestEntry:l?.created_at??null,newestEntry:c?.created_at??null}},"setReasoningCache",0,function(e,n,r,a,o=72e5){a.length>1e4&&(a=a.slice(0,1e4));let s=(0,t.getDbInstance)(),i=Math.floor((Date.now()+o)/1e3),l=a.length;s.prepare(`INSERT OR REPLACE INTO reasoning_cache
       (tool_call_id, provider, model, reasoning, char_count, created_at, expires_at)
     VALUES (?, ?, ?, ?, ?, datetime('now'), ?)`).run(e,n,r,a,l,i)}])},188693,e=>{"use strict";var t=e.i(899378);function n(e){if("number"==typeof e&&Number.isFinite(e))return e;if("string"==typeof e&&e.trim()){let t=Number(e);return Number.isFinite(t)?t:null}return null}function r(e){return null!==e&&Number.isFinite(e)?Math.max(0,Math.min(100,e)):null}function a(e){let t=r(e);return null===t?null:Math.max(0,Math.min(100,100-t))}function o(e,t){return null!==e&&(t<=1&&e>t||e-t>=5)}function s(e){if(!e)return null;let t=Date.parse(e);return Number.isFinite(t)?new Date(t).toISOString():null}function i(e){let t=s(e);return t?t.slice(0,10):null}function l(e,n,c=Date.now()){let u=function(e,n,r=Date.now()){if(!e||!n)return null;let a=i(n);if(!a)return null;let o=(0,t.getDbInstance)(),l=new Date(r).toISOString();try{for(let t of o.prepare(`
        SELECT
      ...
      `).all({connectionId:e,nowIso:l}))if(i(t.windowResetsAt)===a)return s(t.windowStartedAt);return null}catch(e){if(e instanceof Error&&e.message.includes("no such table"))return null;throw e}}(e,n,c),p=function(e,n,l=Date.now()){if(!e||!n)return null;let c=i(n);if(!c)return null;let u=(0,t.getDbInstance)(),p=new Date(l).toISOString();try{let t=u.prepare(`
        SELECT
  ...
  `).run(o,s,i,u,p,r,e,e)},"getCompressionAnalyticsSummary",0,function(e){let n=(0,t.getDbInstance)();a();let r=null;"24h"===e?r=new Date(Date.now()-864e5).toISOString():"7d"===e?r=new Date(Date.now()-6048e5).toISOString():"30d"===e&&(r=new Date(Date.now()-2592e6).toISOString());let o=r?"WHERE timestamp >= ?":"",s=r?[r]:[],i=c(o,"skip_reason IS NULL"),l=n.prepare(`
    SELECT
      COUNT(*) as total,
  ...
  `).all(...s),p=n.prepare(`
    ...
    FROM compression_analytics ${c(o,"skip_reason IS NOT NULL")}
    GROUP BY mode
  ...
  `).all(...s),O={requestsWithReceipts:0,promptTokens:0,completionTokens:0,totalTokens:0,cacheReadTokens:0,cacheWriteTokens:0,estimatedUsdSaved:0,bySource:{}};for(let e of T){let t=e.source??"unknown";O.requestsWithReceipts+=e.cnt,O.promptTokens+=e.prompt,O.completionTokens+=e.completion,O.totalTokens+=e.total,O.cacheReadTokens+=e.cacheRead,O.cacheWriteTokens+=e.cacheWrite,O.estimatedUsdSaved+=e.usdSaved,O.bySource[t]=e.cnt}let I=n.prepare(`
    SELECT COUNT(*) as cnt
  ...
  `).all(...s),N={},C=0;for(let e of h)N[e.reason??"unknown"]=e.cnt,C+=e.cnt;return{totalRequests:l?.total??0,totalTokensSaved:l?.totalSaved??0,avgSavingsPct:Math.round(l?.avgPct??0),avgDurationMs:Math.round(l?.avgDur??0),byMode:d,byEngine:E,byCompressionCombo:g,byProvider:y,last24h:R,totalSkipped:C,bySkipReason:N,validationFallbacks:I?.cnt??0,realUsage:O,mcpDescriptionCompression:{snapshots:D?.cnt??0,estimatedTokensSaved:D?.saved??0}}},"getPerEngineAnalytics",0,function(e,n=7){let r=(0,t.getDbInstance)();a(),i();let o=new Date(Date.now()-864e5*n).toISOString(),s=r.prepare(`SELECT COUNT(*) AS runs,
...
//# sourceMappingURL=_1807lis._.js.map