"""Cantina Discovery Adapter for Direct Work Engine.

Fetches audit competitions from Cantina and converts them to
...
logger = logging.getLogger("ownex.api.direct_work.adapters.cantina")

CANTINA_API_BASE = "https://api.cantina.xyz"


class CantinaDweAdapter(BaseDiscoveryAdapter):
    """Discovers Cantina audit competitions via public API."""

    def __init__(self) -> None:
        source = DiscoverySource(
            name="cantina",
            platform=WorkPlatform.CANTINA,
            categories=[
    ...
    async def fetch_opportunities(self) -> list[Opportunity]:
        """Fetch active Cantina competitions and convert to Opportunities."""
        opportunities: list[Opportunity] = []

        try:
            async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
                endpoints = [
                    "https://api.cantina.xyz/v1/competitions",
                ...
                for endpoint in endpoints:
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            if competitions:
                                logger.info("Cantina: successfully fetched from %s", endpoint)
                                break
                    except Exception as e:
                        logger.debug("Cantina endpoint %s failed: %s", endpoint, e)
                        continue

                if not competitions:
                    logger.warning("Cantina: all endpoints failed")
            ...
            logger.error("Error fetching Cantina competitions: %s", e)
    ...
    async def validate_connection(self) -> bool:
        try:
            async with httpx.AsyncClient(timeout=10, follow_redirects=True) as client:
                resp = await client.get("https://api.cantina.xyz/v1/competitions")
                return resp.status_code == 200
        except Exception:
            return False

    def _convert_competition(self, comp: dict[str, Any]) -> Opportunity | None:
        """Convert Cantina competition to Opportunity with audit-specific barrier assessment."""
        try:
            ...
            return self._create_opportunity(
                external_id=comp_id,
                title=f"{name} — Cantina Audit",
                category=OpportunityCategory.SECURITY_RESEARCH,
                url=f"https://cantina.xyz/competitions/{comp_id}",
            ...
            logger.error("Error converting Cantina competition %s: %s", comp.get("id"), e)
            return None
    ...
    def _extract_tags(self, comp: dict[str, Any]) -> list[str]:
        tags = ["web3", "smart_contract_audit", "cantina", "security_research", "defi"]
        if comp.get("language"):
...
def build_cantina_adapter() -> CantinaDweAdapter:
    """Factory function for building the Cantina adapter."""
    return CantinaDweAdapter()
