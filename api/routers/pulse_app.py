"""Pulse App API — AI Work platforms (Outlier, DataAnnotation, Mindrift, Remotasks)."""
...
def list_platforms():
    """List available Pulse platforms."""
    return {"platforms": PULSE_PLATFORMS}
...
def list_opportunities(
    ...
    query = db.query(CycleOpportunity).filter(CycleOpportunity.cycle_slug == "pulse")
    if platform:
...
def list_sync_logs(
    ...
    query = db.query(CycleSyncLog).filter(CycleSyncLog.cycle_slug == "pulse")
    if platform:
...
def trigger_sync(platform: str | None = Query(None), db: Session = Depends(get_db)):
    """Trigger a sync for one or all Pulse platforms."""
    platforms = [platform] if platform else PULSE_PLATFORMS
    logs = []
    for p in platforms:
        log = CycleSyncLog(cycle_slug="pulse", platform=p, status="queued")
        db.add(log)
        logs.append(log)
    db.commit()
    return {
