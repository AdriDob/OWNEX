"""Forge App API — Dev Bounty platforms (Superteam, Opire, Algora, IssueHunt)."""
...
def list_platforms():
    """List available Forge platforms."""
    return {"platforms": FORGE_PLATFORMS}
...
def list_opportunities(
    ...
    query = db.query(CycleOpportunity).filter(CycleOpportunity.cycle_slug == "forge")
    if platform:
...
def list_sync_logs(
    ...
    query = db.query(CycleSyncLog).filter(CycleSyncLog.cycle_slug == "forge")
    if platform:
...
def trigger_sync(platform: str | None = Query(None), db: Session = Depends(get_db)):
    """Trigger a sync for one or all Forge platforms."""
    platforms = [platform] if platform else FORGE_PLATFORMS
    logs = []
    for p in platforms:
        log = CycleSyncLog(cycle_slug="forge", platform=p, status="queued")
        db.add(log)
        logs.append(log)
    db.commit()
    return {
