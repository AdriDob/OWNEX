"""Atlas App API — Research/Intelligence platforms (CVE, ExploitDB, GitHub Advisory, Google Trends, Shodan)."""
...
ATLAS_PLATFORMS = ["cve", "exploitdb", "github_advisory", "google_trends", "shodan"]
...
def list_platforms():
    """List available Atlas platforms."""
    return {"platforms": ATLAS_PLATFORMS}
...
def list_opportunities(
    ...
    query = db.query(CycleOpportunity).filter(CycleOpportunity.cycle_slug == "atlas")
    if platform:
...
def list_sync_logs(
    ...
    query = db.query(CycleSyncLog).filter(CycleSyncLog.cycle_slug == "atlas")
    if platform:
...
def trigger_sync(platform: str | None = Query(None), db: Session = Depends(get_db)):
    """Trigger a sync for one or all Atlas platforms."""
    platforms = [platform] if platform else ATLAS_PLATFORMS
    logs = []
    for p in platforms:
        log = CycleSyncLog(cycle_slug="atlas", platform=p, status="queued")
        db.add(log)
        logs.append(log)
    db.commit()
    return {
