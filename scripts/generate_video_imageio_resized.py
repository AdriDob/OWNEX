"""Generate video using imageio with image resizing.

Resizes all images to 1920x1080 before creating video.
"""
...
def generate_video_with_imageio():
    """Generate video using imageio with resizing."""
    try:
        import imageio
    ...
    try:
        ...
        for img_path, duration in zip(IMAGES, DURATIONS):
            ...
            print(f"✓ Processed: {img_path} ({duration}s, {frames_count} frames)")
...
if __name__ == "__main__":
    print("OWNEX Video Generator using imageio with resizing")
    print("=" * 50)
