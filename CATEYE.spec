...
# ruff: noqa
#
# CATEYE — PyInstaller build spec.
# Single entrypoint: run.py → desktop/main_desktop.py → FastAPI + pywebview.
# Output: dist/CATEYE/CATEYE.exe (or dist/CATEYE/CATEYE on Linux)
#
# Usage:
#   pyinstaller CATEYE.spec -y                 # one-dir build (Windows 11 recommended)
#   pyinstaller CATEYE.spec -y --onefile       # single .exe (portable, Windows only)
...
ICON_PATH = str(PROJECT_ROOT / "installer" / "icons" / "cateye.ico")
