...
logger = logging.getLogger("catseye.agents.research")
...
class ResearchAgent(BaseAgent):
    ...
    def _run_discovery(self, event: AgentEvent) -> None:
        ...
        if target_name:
            try:
                from cores.recon.subfinder_runner import run_subfinder
        ...
        if endpoints:
            try:
                from cores.recon.httpx_runner import run_httpx
        ...
        if endpoint_paths := [e["path"] for e in endpoints[:20]]:
            try:
                from cores.recon.katana_runner import run_katana
