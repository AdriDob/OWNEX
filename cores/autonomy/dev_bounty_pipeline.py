...
from cores.autonomy.pr_builder import PRBuilder, PRResult
...
from cores.opportunity.executors import ExecutionResult
...
class DevBountyPipeline:
    ...
    async def execute_dev_bounty(
        ...
        """Execute full dev bounty pipeline end-to-end (records outcome in RevenueTracker)."""
        result = await self._execute_dev_bounty_inner(
            bounty_id, platform, repo_url, issue_number, issue_url, title, description
        )
        try:
            from cores.revenue_tracker.execution_bridge import record_pipeline_outcome
        ...
        except Exception as exc:  # pragma: no cover - revenue never blocks execution
            logger.warning("Revenue bridge skipped for %s: %s", bounty_id, exc)
        return result

    async def _execute_dev_bounty_inner(
        self,
        bounty_id: str,
        platform: str,
        repo_url: str,
