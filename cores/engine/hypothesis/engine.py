...
from cores.engine.hypothesis.llm import detect_gaps, enrich_reasoning, refine_priority
from cores.engine.hypothesis.memory import HypothesisMemory
...
class HypothesisEngine:
    ...
    def run(
        ...
        zap_alerts: list[dict[str, Any]] | None = None,
        ...
        if zap_h:
            hypotheses.extend(zap_h)
            LOG.info("Stage ZAP (passive alerts): %d hypotheses from ZAP alerts", len(zap_h))
    ...
    def _stage_zap(
        ...
        target_id: int,
        target_name: str,
        zap_alerts: list[dict[str, Any]] | None,
        endpoint_map: dict[str, int] | None = None,
    ) -> list[Hypothesis]:
        if not zap_alerts:
            return []
