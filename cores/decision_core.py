...
from .knowledge_core import KnowledgeGraph, TaskType, OutcomeType, get_knowledge_graph
from .operations_core import TaskCandidate, TaskPriority
...
class Decision:
    ...
    selected_task: Optional[TaskCandidate]
    selected_agent: str
...
class DecisionEngine:
    ...
    def decide(self, context: DecisionContext) -> Decision:
        ...
        for candidate in feasible:
            for agent in self._get_available_agents(candidate, context):
                ...
                score = self._score_by_policy(
                    ev, conf, details, candidate, agent, context
                )
        ...
        alternatives = [
            (c, a, ev) for _, c, a, ev, _, _ in evaluations[1:6]
        ]
        
        rationale = self._build_rationale(
            best_candidate, best_agent, best_ev, best_conf, best_details, context.policy
        )
    ...
    def _score_by_policy(
        ...
        if policy == DecisionPolicy.MAX_EXPECTED_VALUE:
            ...
            sampled_ev = (belief.success_probability * sampled_reward 
                         - belief.expected_cost) * candidate.confidence
            return sampled_ev
...
# Factory functions
def create_decision_engine(kg: Optional[KnowledgeGraph] = None) -> DecisionEngine:
    return DecisionEngine(kg)


def create_bayesian_bandit(kg: Optional[KnowledgeGraph] = None) -> BayesianBandit:
