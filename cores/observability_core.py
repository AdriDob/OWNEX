...
class AgentExecution:
    ...
    def complete(self, outcome: str, reward: float = 0.0, error: Optional[str] = None, category: Optional[str] = None):
...
class MetricsCollector:
    ...
    def timer(self, name: str, duration_seconds: float, labels: Optional[Dict[str, str]] = None):
        with self._lock:
    ...
    def record_execution(self, execution: AgentExecution):
        ...
        with self._lock:
            ...
            if execution.outcome in ("failure", "error", "timeout"):
                self.increment("executions_failed", labels={"agent": execution.agent, "category": execution.error_category or "unknown"})
    
    def create_alert(self, severity: AlertSeverity, title: str, message: str, source: str, metadata: Dict = None) -> Alert:
        alert = Alert(
