...
class DecisionRecord:
    ...
    actual_outcome: Optional[TaskOutcome] = None
...
class KnowledgeGraph:
    ...
    def _load_caches(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in conn.execute(
                "SELECT * FROM outcomes ORDER BY completed_at DESC LIMIT 1000"
            ):
    ...
    def _update_platform_expertise(self, conn: sqlite3.Connection, outcome: TaskOutcome):
        ...
        plat.avg_duration = (
            (plat.avg_duration * (plat.total_tasks - 1) + outcome.duration_seconds) 
            / plat.total_tasks
        )
        ...
        if outcome.failure_reason:
            plat.common_failures[outcome.failure_reason] = \
                plat.common_failures.get(outcome.failure_reason, 0) + 1
        ...
        """, (
            plat.platform, plat.total_tasks, plat.successful_tasks,
            plat.total_reward, plat.total_cost, plat.avg_duration,
            json.dumps(plat.best_agents), json.dumps(plat.best_task_types),
            json.dumps(plat.peak_hours), json.dumps(plat.common_failures),
    ...
    def _update_agent_profile(self, conn: sqlite3.Connection, outcome: TaskOutcome):
        ...
        # Failure patterns
        if outcome.failure_reason:
            agent.failure_patterns[outcome.failure_reason] = \
                agent.failure_patterns.get(outcome.failure_reason, 0) + 1
        ...
        """, (
            agent.agent, agent.total_tasks, agent.successful_tasks,
            agent.total_reward, agent.total_cost, agent.avg_duration,
            json.dumps(agent.platform_affinity), json.dumps(agent.task_type_affinity),
            json.dumps(agent.failure_patterns), agent.last_updated.isoformat()
        ))
    ...
    def get_platform_expertise(self, platform: str) -> Optional[PlatformExpertise]:
    ...
    def get_best_agent_for(self, platform: str, task_type: TaskType) -> Optional[str]:
    ...
    def get_recent_failures(self, platform: str = None, limit: int = 20) -> List[TaskOutcome]:
        failures = [o for o in self._recent_outcomes 
