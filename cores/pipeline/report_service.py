...
from cores.pipeline.stages import PipelineContext, PipelineStage
...
def generate_and_save_report(
    ...
    from cores.reporting.reporting import ReportGenerator
...
def _report_to_dict(r: models.Report) -> dict[str, Any]:
    content = _safe_json_loads(r.content, {})
    return {
        "id": r.id,
        "investigation_id": r.investigation_id,
        "format": r.format,
        "content": content,
        "finding_ids": _safe_json_loads(r.finding_ids, []),
        ...
        "notes": r.notes or "",
        "timeline": _safe_json_loads(r.timeline, []),
        "attachments": _safe_json_loads(r.attachments, []),
