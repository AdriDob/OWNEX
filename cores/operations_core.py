...
from .knowledge_core import (
    KnowledgeGraph, TaskType, TaskOutcome, OutcomeType,
    PlatformExpertise, AgentProfile, get_knowledge_graph
)
...
class OperationsResearchEngine:
    ...
    def plan(
        ...
        candidates: Optional[List[TaskCandidate]] = None
    ) -> List[ScheduledTask]:
        """
    ...
    def _schedule_greedy(
        self,
        scored: List[Tuple[float, TaskCandidate]],
        budget: ResourceBudget
    ) -> List[ScheduledTask]:
        ...
        for score, candidate in scored:
            ...
            concurrent = sum(1 for s in schedule 
    ...
    def _calculate_start_time(
        ...
        earliest = max(
            timelines[agent],
            budget.time_window_start or datetime.utcnow()
        )
...
class ContinuousPlanner:
    ...
    def get_next_action(self) -> Optional[ScheduledTask]:
