...
from cores.targets import parser
from cores.targets.models import Scope, TargetIntel
from cores.targets.technology import (
    classify_cms,
    fingerprint_program,
...
class Hunter:
    ...
    def ingest_programs(self, programs: list[dict]) -> list[dict]:
        ...
        try:
            for p in programs:
                if p.get("has_rewards") is False:
                    LOG.debug("Skipping non-reward program: %s", p.get("name") or p.get("title") or "unnamed")
                    continue
