...
from cores.knowledge_core import (
    KnowledgeGraph, TaskOutcome, TaskType, OutcomeType, get_knowledge_graph
)
from cores.operations_core import (
    OperationsResearchEngine, TaskCandidate, TaskPriority, ResourceBudget,
    create_default_budget, create_candidate, ContinuousPlanner
)
from cores.decision_core import (
    DecisionEngine, DecisionContext, DecisionPolicy, RiskProfile,
    BayesianBandit, default_decision_context
)
from cores.observability_core import (
    MetricsCollector, ExecutionTracker, AgentExecution, get_metrics_collector, get_execution_tracker
)
...
def test_operations_core(kg):
    ...
    # Crear candidatos
    candidates = [
        create_candidate("scan_001", TaskType.SCAN, "hackerone", "Scan new program", 30, 2.0, 500, 0.8, TaskPriority.HIGH),
        create_candidate("exploit_001", TaskType.EXPLOIT, "hackerone", "Exploit found vuln", 60, 5.0, 2000, 0.6, TaskPriority.CRITICAL),
        create_candidate("recon_001", TaskType.RECON, "bugcrowd", "Recon new target", 20, 1.0, 100, 0.7, TaskPriority.MEDIUM),
    ...
    for s in schedule:
        print(f"  {s.candidate.task_id} -> {s.assigned_agent} @ {s.start_time.strftime('%H:%M')}-{s.end_time.strftime('%H:%M')} (EV: ${s.expected_value:.2f})")
...
def test_decision_core(kg):
    ...
    candidates = [
        ...
        create_candidate("task_b", TaskType.EXPLOIT, "hackerone", "Critical exploit", 60, 10.0, 5000, 0.4, TaskPriority.CRITICAL),
    ...
    arms = [("hackerone", TaskType.SCAN, "opencode"), ("hackerone", TaskType.EXPLOIT, "hermes"), ("bugcrowd", TaskType.RECON, "fcc")]
...
def test_integrated_flow():
    ...
    for cycle in range(3):
        ...
        candidates = [
            ...
            create_candidate(f"exploit_{cycle}_a", TaskType.EXPLOIT, "hackerone", f"Exploit cycle {cycle}", 45, 5.0, 1000, 0.5, TaskPriority.HIGH),
        ...
        print(f"Decision: {decision.selected_task.task_id} with {decision.selected_agent} (EV: ${decision.expected_value:.2f})")
        ...
        for scheduled in schedule:
            with tracker.track(
                ...
                from cores.knowledge_core import TaskOutcome, OutcomeType
                outcome = TaskOutcome(
