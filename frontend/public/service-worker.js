...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) => {
      return Promise.all(
        keys.filter((k) => k !== CACHE_STATIC && k !== CACHE_API && k !== CACHE_INTEL).map((k) => caches.delete(k)),
      )
    }),
  )
  self.clients.claim()
...
function isStaticAsset(url) {
  return STATIC_ASSETS.some((a) => url.endsWith(a)) || url.match(/\.(js|css|woff2?|png|svg|ico|json)$/)
}
