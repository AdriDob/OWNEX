...
const ROUTES = [
  { path: '/', name: 'welcome', waitFor: '.hero, .welcome, h1', desc: 'Página de bienvenida' },
  {
    path: '/dashboard',
    name: 'gaming-console',
    waitFor: '.gaming-console, .dashboard, [data-testid="dashboard"]',
    desc: 'Gaming Console / Dashboard principal',
  },
  {
    path: '/classic',
    name: 'mission-control',
    waitFor: '.mission-control, .agent-fleet, .opportunity-radar',
    desc: 'Mission Control clásico',
  },
  {
    path: '/intelligence',
    name: 'intelligence',
    waitFor: '.intelligence, .findings, .hypothesis',
    desc: 'Intelligence Dashboard',
  },
  ...
  { path: '/capital', name: 'capital', waitFor: '.capital, .revenue, .wealth', desc: 'Capital / Wealth Dashboard' },
  {
    path: '/operations',
    name: 'operations',
    waitFor: '.operations, .terminal, .workflows',
    desc: 'Operations & Terminal',
  },
  { path: '/copilot', name: 'copilot', waitFor: '.copilot, .merlin, .chat', desc: 'MERLIN Copilot' },
  {
    path: '/security/executive',
    name: 'executive-dashboard',
    waitFor: '.executive, .ceo-view, .verdict',
    desc: 'Executive Dashboard (CEO View)',
  },
...
async function main() {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'capture-summary.json'), JSON.stringify(summary, null, 2))
