"""Atlas Cycle — Intelligence, Market Analysis, Predictive Insights.

Coordinates: Collect -> Process -> Analyze -> Predict -> Alert -> Learn
"""
...
logger = logging.getLogger("ownex.cycles.atlas")
...
class AtlasCycle:
    ...
    Stages:
    1. COLLECT — data ingestion, source monitoring, signal gathering
    2. PROCESS — cleaning, normalization, enrichment, feature extraction
    3. ANALYZE — pattern detection, correlation analysis, trend identification
    4. PREDICT — forecasting, scenario modeling, probability estimation
    5. ALERT — threshold monitoring, anomaly detection, notification dispatch
    6. LEARN — model evaluation, accuracy tracking, methodology refinement
    """
    ...
    def ensure_cycle(self) -> Cycle:
        """Ensure the Atlas cycle exists in DB."""
        cycle = self._cycle_service.get_by_slug("atlas")
        if not cycle:
            cycle = self._cycle_service.create(
                {
                    "name": "Atlas",
                    "slug": "atlas",
                    "description": "Intelligence, market analysis, predictive insights",
                    "category": "intelligence",
                    "enabled": True,
                    "priority": 5,
                    "status": "idle",
                    "config": {
                        "sources": ["coingecko", "polymarket", "news", "onchain"],
                        "prediction_horizon": "24h",
                        "confidence_threshold": 0.7,
                    },
                }
            )
            logger.info("Created Atlas cycle")
        return cycle

    def start_cycle(self) -> Cycle:
        """Start the Atlas cycle."""
        ...
        activated = self._cycle_service.activate(cycle.id, next_action="collect")
        logger.info("Atlas cycle started")
        return activated
    ...
    def capture_learning(self, prediction_id: int) -> Any:
        """Capture knowledge from a prediction outcome."""
        return None
...
_ATLAS_CYCLE: AtlasCycle | None = None


def get_atlas_cycle() -> AtlasCycle:
    """Get the global AtlasCycle instance."""
    global _ATLAS_CYCLE
    if _ATLAS_CYCLE is None:
        _ATLAS_CYCLE = AtlasCycle()
    return _ATLAS_CYCLE


# Register Atlas cycle in CycleRegistry
def register_atlas_cycle(registry) -> None:
    """Register Atlas cycle definition."""
    import contextlib
