"""Mindrift Browser Worker — UI automation for Mindrift.io tasks."""
...
class MindriftBrowserWorker:
    """Worker for Mindrift.io — login, claim tasks, submit work via browser."""
    ...
    async def login(self, email: str = "", password: str = "") -> dict[str, Any]:
        result = await self.browser.goto("https://mindrift.io/login")
    ...
    async def submit_task(self, task_id: str, solution_url: str = "", description: str = "") -> dict[str, Any]:
        if solution_url:
            await self.browser.fill('input[type="url"]', solution_url)
        if description:
            await self.browser.fill("textarea", description)
    ...
    async def execute(self, action: str, **kwargs: Any) -> dict[str, Any]:
        ...
        if action == "submit_task":
            return await self.submit_task(
                kwargs.get("task_id", ""), kwargs.get("solution_url", ""), kwargs.get("description", "")
            )
