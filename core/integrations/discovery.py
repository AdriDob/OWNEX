...
BUILTIN_INTEGRATIONS: list[IntegrationDef] = [
    ...
    # ── Plugin-Based External Tools (loaded via extensions/) ──
    IntegrationDef(
        "playwright",
        "sensor",
        "Web navigation and browser automation sensor",
        icon="🌐",
        health_check="extensions.playwright.connector.PlaywrightConnector",
        docs_url="https://github.com/microsoft/playwright",
        tags=["plugin", "web", "sensor", "browser"],
        pending_manual_note="pip install playwright async-playwright",
    ),
    IntegrationDef(
        "aider",
        "editor",
        "AI-assisted code editing and repository management",
        icon="✏️",
        health_check="extensions.aider.connector.AiderConnector",
        docs_url="https://github.com/Aider-AI/aider",
        tags=["plugin", "code-editor", "git"],
        pending_manual_note="pip install aider-chat",
    ),
    IntegrationDef(
        "mcp",
        "bridge",
        "Model Context Protocol bridge for external tool access",
        icon="🔌",
        health_check="extensions.mcp.connector.MCPBridgeConnector",
        docs_url="https://github.com/modelcontextprotocol",
        tags=["plugin", "mcp", "bridge", "tools"],
        pending_manual_note="pip install mcp",
    ),
    IntegrationDef(
        "qdrant",
        "storage",
        "Vector memory and semantic search storage",
        icon="🧠",
        health_check="extensions.qdrant.connector.QdrantConnector",
        docs_url="https://github.com/qdrant/qdrant",
        tags=["plugin", "vector", "memory", "storage", "rag"],
        pending_manual_note="pip install qdrant-client and run Qdrant container",
    ),
    IntegrationDef(
        "git",
        "version-control",
        "Git automation for commit, diff, and PR management",
        icon="📂",
        health_check="extensions.git.connector.GitConnector",
        docs_url="https://github.com/gitpython-developers/GitPython",
        tags=["plugin", "git", "automation", "vcs"],
        pending_manual_note="pip install GitPython",
    ),
]
