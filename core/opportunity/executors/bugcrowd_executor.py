"""Bugcrowd Executor — Submit reports, claim bounties on Bugcrowd."""
...
class BugcrowdExecutor(BaseExecutor):
    """Executor for Bugcrowd — submit reports, claim bounties."""

    platform = "bugcrowd"

    def __init__(self, config: dict[str, Any] | None = None):
        super().__init__(config)
        self.token = self.config.get("token") or os.getenv("BUGCROWD_API_KEY")
        self.base_url = self.config.get("base_url", "https://api.bugcrowd.com")

    async def execute(self, action: str, **kwargs: Any) -> ExecutionResult:
        if action == "submit_report":
            return await self.submit_report(
                kwargs.get("program_code") or "",
                kwargs.get("title") or "",
        ...
        if action == "get_program":
            return await self.get_program(kwargs.get("program_code") or "")
    ...
    async def submit_report(
        ...
        if not self.token:
            return ExecutionResult(False, "submit_report", "", error="BUGCROWD_API_KEY not configured")

        if not program_code:
            return ExecutionResult(False, "submit_report", "", error="Program code required")
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if resp.status_code in (200, 201):
                        ...
                        return ExecutionResult(
                            ...
                            "submit_report",
                            "",
                            f"Report submitted to {program_code}",
    ...
    async def claim_bounty(self, submission_id: str) -> ExecutionResult:
        """Claim bounty for an accepted submission."""
        if not self.token:
            return ExecutionResult(False, "claim_bounty", submission_id, error="BUGCROWD_API_KEY not configured")
    ...
    async def get_submission(self, submission_id: str) -> ExecutionResult:
        if not self.token:
            return ExecutionResult(False, "get_submission", "", error="BUGCROWD_API_KEY not configured")
    ...
    async def get_program(self, program_code: str) -> ExecutionResult:
        if not self.token:
            return ExecutionResult(False, "get_program", program_code, error="BUGCROWD_API_KEY not configured")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if resp.status_code == 200:
                    data = resp.json()
                    return ExecutionResult(True, "get_program", program_code, data=data)
                return ExecutionResult(
                    False, "get_program", program_code, error=f"HTTP {resp.status_code}: {resp.text[:200]}"
                )
            except Exception as e:
                return ExecutionResult(False, "get_program", program_code, error=str(e))

    async def health_check(self) -> ExecutionResult:
        if not self.token:
            return ExecutionResult(False, "health_check", self.base_url, error="BUGCROWD_API_KEY not configured")
        async with httpx.AsyncClient() as client:
            try:
                resp = await client.get(f"{self.base_url}/ping", timeout=15)
