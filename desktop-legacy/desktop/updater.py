...
def _get_exe_dir() -> Path:
    ...
    return Path(__file__).resolve().parent.parent / "build" / "release" / "CATEYE"
...
def check_for_updates(current_version: str | None = None) -> ReleaseInfo | None:
    ...
    try:
        req = Request(api_url, headers={"User-Agent": "CATEYE/1.0", "Accept": "application/json"})
...
def download_update(release: ReleaseInfo) -> str | None:
    ...
    dest = UPDATE_STAGING_DIR / f"cateye-{release.version}.zip"
    ...
    try:
        req = Request(release.download_url, headers={"User-Agent": "CATEYE/1.0"})
...
def apply_update(release: ReleaseInfo, downloaded_path: str) -> bool:
    ...
    extracted_exe = None
    for candidate in ["CATEYE.exe", "CATEYE", "run.exe", "run"]:
